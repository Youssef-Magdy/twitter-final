        ...
        <div class="p-6 bg-white border-b border-gray-200">
          ...
          <div class="continer">
            <div class="card border-primary mb-3">
              <div class="card-header bg-transparent border-primary text-primary">Added By : {{ $post->user->name }}</div>
              <div class="card-body text-success">
                <h5 class="card-title"> {{ $post->title }}</h5>
                <p class="card-text"> {{ $post->body }} </p>

                <form action=" {{ ($post->likedPostUser->count() > 0 ? route('liked.destroy' , $post->id) :route('liked.store') )}} " method="POST">
                  @csrf
                  @if ($post->likedPostUser->count() > 0)
                  @method('DELETE')
                  @endif
                  <input type="hidden" value="{{Auth::user()->id}}" name="user_id">
                  <input type="hidden" value="{{ $post->id }}" name="post_id">

                  <button type="submit" class="btn-o btn-primary position-relative float-end "><span class="h4 text-danger"><i class="{{ ($post->likedPostUser->count() > 0 ? 'bi bi-chat-heart-fill' : 'bi bi-chat-heart')}} "></i></span>
                    <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger">
                      {{ $post->likedPost->count() }}
                      ...
                      <div class="flex-grow-1 flex-shrink-1">
                        ...
                        <form action="{{ route('posts.store') }}" method="post">
                          ...
                          <div class="row align-items-center">
                            ...
                            <div class="col-11">
                              <textarea class="form-control" placeholder="Leave a Reply here" name="body" id="floatingTextarea"></textarea>
                            </div>
                            <div class="col-1">
                              <button type="submit" class="btn-o btn-success"><span class="h2 text-success"><i class="bi bi-reply"></i></span></button>
                            </div>
                          </div>
                        </form>
                        @foreach ($comment->comment as $replay )
                ...
                <div class="form-floating">
                  ...
                  <form action="{{ route('posts.store') }}" method="post">
                    ...
                    <div class="row align-items-center">
                      ...
                      <div class="col-1">
                        <button type="submit" class="btn-o btn-success"><span class="h2 text-success"><i class="bi bi-send"></i></span></button>
                      </div>
                    </div>
                  </form>
